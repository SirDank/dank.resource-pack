...
from dankware import cls, clr, rm_line, white, red, multithread, blue_bright, green_bright
    ...
    for key, value in sorted(dict2.items(), key=lambda x: x[0]):
        if key in dict1 and isinstance(dict1[key], dict) and isinstance(value, dict):
            ...
            if key in dict1 and dict1[key] != value:
                print(clr(f"  [ {current_zip_file} | {current_json_file} ] Overwriting: {key} [{dict1[key]}] with [{value}]", colour_two=green_bright))
            dict1[key] = value
    return dict1
...
def copy_and_overwrite(src, dst):
    ...
    for item in os.listdir(src):
        ...
        if os.path.isdir(src_item):
            ...
            dst_item_stripped = os.path.abspath(dst_item).split("dank.resource-pack\\")[-1]
            if dst_item.endswith(".json"):
                if os.path.isfile(dst_item):
                    current_json_file = dst_item_stripped
                    ...
                    # print(clr(f"  - Merged [ {dst_item_stripped} ]"))
                    continue
            if os.path.isfile(dst_item) and not os.path.isfile(os.path.join("base", dst_item_stripped)):
                print(clr(f"  [ {current_zip_file} ] Overwrote [ {dst_item_stripped} ]", colour_two=blue_bright))
...
if __name__ == "__main__":
    ...
    print(clr("\n  - Extracting Zips...\n"))
    extract_zips()
    print(clr("\n  - Copying and Overwriting with Base Assets..."))
